...
class ShowHealthPoints
{
	...
	void UpdateInfo(CBasePlayer@ pPlayer, string sHP)
	{
		...
		lifetime = Globals.GetCurrentTime() + 0.2f;
		Show(pPlayer);
	}
...
class CSZMPlayer
{
	...
	private TimeBasedDamage@ pBleeding;
	...
	void RespawnLobby()
	{
		if (LobbyRespawnDelay <= Globals.GetCurrentTime() && FindEntityByEntIndex(PlayerIndex).GetTeamNumber() == TEAM_LOBBYGUYS) 
		{
	...
	void Think()
	{
		...
		if (pBleeding !is null)
		{
			pBleeding.Think(FindEntityByEntIndex(PlayerIndex));

			if (pBleeding.Tiks == 0 || pPlayerEntity.GetTeamNumber() != TEAM_ZOMBIES)
			{
				@pBleeding = null;
			}
		}
		...
		{
			if (RecoverTime <= Globals.GetCurrentTime() && RecoverTime != 0)
			{
				RecoverTime = Globals.GetCurrentTime() + flRecover;
			...
			if (TeamNum == TEAM_ZOMBIES)
			{
				...
				if (RegenTime <= Globals.GetCurrentTime() && pPlayerEntity.GetHealth() < pPlayerEntity.GetMaxHealth())
				{
			...
			else if (TeamNum == TEAM_SURVIVORS)
			{
				...
				if (IRITime <= Globals.GetCurrentTime() && Utils.StrContains("item_antidote", pPlayer.GetCurrentWeapon().GetEntityName()))
				{
					IRITime = Globals.GetCurrentTime() + 1.12f;
					string sMax = "";
...
void OnEntityPickedUp(CZP_Player@ pPlayer, CBaseEntity@ pEntity)
{
	...
	if (Utils.StrEql("item_antidote", pEntity.GetEntityName(), true))
	{
		...
		if (pCSZMPlayer.InfectResist >= CONST_MAX_INFECTRESIST)
		{
...
HookReturnCode CSZM_OnPlayerDamaged(CZP_Player@ pPlayer, CTakeDamageInfo &out DamageInfo)
{
	if (bIsCSZM)
	{
		...
		{
			if ((Utils.StrEql(pEntityAttacker.GetClassname(), "worldspawn") && bDamageType(iDamageType, 0)) || (Utils.StrEql(pEntityAttacker.GetEntityName(), "frendly_shrapnel") && iVicTeam == TEAM_SURVIVORS) || (iAttTeam == iVicTeam && iAttIndex != iVicIndex))
			{
		...
		if (iVicTeam == TEAM_ZOMBIES && pBaseEnt.IsAlive())
		{
			...
			if (flDamage < pBaseEnt.GetHealth() && flDamage >= 1.0f && !bDamageType(iDamageType, 14) && (iAttTeam != iVicTeam && iAttIndex != iVicIndex))
			{
				//ZM ViewPunch
				bool bLeft = false;
				float VP_X = 0;
