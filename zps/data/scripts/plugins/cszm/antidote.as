...
void OnEntityPickedUp(CZP_Player@ pPlayer, CBaseEntity@ pEntity)
{
	...
	if(pEntity.GetEntityName() == "iantidote")
	{
		...
		else Engine.Ent_Fire(iIndex + "iantidote", "addoutput", "itemstate 1");
	}	
}

void OnEntityDropped(CZP_Player@ pPlayer, CBaseEntity@ pEntity)
{
    if(pPlayer is null) SD("pPlayer is {blue}null");
    if(pEntity is null) SD("pEntity is {blue}null");
...
void OnItemDeliverUsed(CZP_Player@ pPlayer, CBaseEntity@ pEntity, int &in iEntityOutput)
{
	...
	if(Utils.StrContains("iantidote", pEntity.GetEntityName())) AntidoteInjection(iIndex, pPlayer, pEntity);
}
...
void SetAntidoteState(const int &in iIndex, const int &in iAStage)
{
	...
	{
		if(Utils.StringToInt(pEntity.GetEntityName()) == iIndex && Utils.StrContains("iantidote", pEntity.GetEntityName())) bTest = true;
	}
